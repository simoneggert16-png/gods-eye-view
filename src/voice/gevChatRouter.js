...
export const ROUTER_TOOLS = Object.freeze([
  ...
  'web_search {query} — AUTONOMOUS LIVE INTERNET SEARCH: look up facts, people, celebrities, private estates/mansions, addresses, background research, news, or events when asked to research ("recherchiere...", "suche nach...", "wer ist...", "wo wohnt...", "finde heraus...") or when exact real-world address/coordinates are unknown. Returns live web & encyclopedia results, which you can then navigate to or summarize in German.',
...
export const ROUTER_SYSTEM_PROMPT = [
  ...
  + '- If you know the exact verified coordinates or street address with high certainty, use them immediately (latitude and longitude as numbers)!\n'
  + '- IF YOU DO NOT KNOW THE EXACT REAL-WORLD ADDRESS OR COORDINATES WITH HIGH CERTAINTY, OR IF THE USER ASKS TO RESEARCH ("recherchiere...", "finde heraus...", "suche im Web..."): DO NOT GUESS OR HALLUCINATE COORDINATES! Emit a web_search tool call FIRST (e.g. {"name": "web_search", "args": {"query": "Sean Diddy Combs villa address Los Angeles"}, "say": "Ich recherchiere Diddys Villa im Internet..."}). The system executes the search and supplies you with the real address and coordinates in the follow-up step!\n'
  + '- For annotate_map: When coordinates are verified, provide latitude and longitude (as numbers) inside the annotation object alongside target, e.g. {"name": "annotate_map", "args": {"annotations": [{"type": "area", "target": "Diddys Villa", "latitude": 34.0788, "longitude": -118.4312, "entityKind": "building"}], "flyTo": true}, "say": "Ich markiere Diddys Villa in Los Angeles."}.\n'
  + '- For fly_to_location: When coordinates are verified, supply latitude and longitude (as numbers), with appropriate rangeM (e.g. 500-1200 for villas/estates/compounds, 2500 for neighborhoods, 15000 for cities), pitch (e.g. -25 to -35 for 3D perspective), and heading! (e.g. {"name": "fly_to_location", "args": {"latitude": 34.0788, "longitude": -118.4312, "rangeM": 600, "pitch": -30}, "say": "Ich fliege zu Diddys Villa in Los Angeles."}).\n'
  ...
  'SPECIFIC BUILDINGS, HOSPITALS, CAMPUSES & INSTITUTIONS ("kantonsspital st.gallen", "ksh st.gallen", "universitätsspital", "eth zürich", "stade de suisse", schools, hospitals, local landmarks): When the user asks for a specific named building, hospital, campus, or facility whose exact building-level meter coordinates you do not know with 10-meter precision, ALWAYS supply query: "<full place name>" (e.g. {"name": "fly_to_location", "args": {"query": "Kantonsspital St. Gallen"}, "say": "Ich zeige das Kantonsspital St. Gallen."}) instead of guessing city-center coordinates! This allows the high-precision geocoder and 3D building detection to frame the real campus cleanly at a safe altitude.',
  ...
  'AUTONOMOUS LIVE INTERNET RESEARCH & KNOWLEDGE-THEN-ACT: You have full live internet access via web_search! Whenever the user asks to research ("recherchiere...", "suche im Internet...", "finde heraus...", "wer ist...", "was ist mit... passiert?"), OR asks for a person\'s residence/mansion/villa, private estate, crime scene, or an event/incident whose exact facts or address you do not know with 100% certainty: ALWAYS FIRST emit web_search with a concise, targeted search query (e.g. {"name": "web_search", "args": {"query": "Sean Diddy Combs villa address Los Angeles"}, "say": "Ich recherchiere die Adresse im Web..."}). The system runs the live web search and returns the top snippets to you. In the follow-up, you will either navigate/mark the discovered location (fly_to_location / annotate_map) or provide an intelligent, comprehensive German direct answer!',
